...
function currentEntry() {
  ...
  return {
    ...
    mode,
    id,
    season: readInteger(seasonInput.value, "Season", 0),
    episode: readInteger(episodeInput.value, "Episode", 1)
  };
}
...
function currentEntrySafe() {
  try {
    ...
    return {
      ...
      id: mediaIdInput.value || "1",
      season: Number.parseInt(seasonInput.value, 10) || 1,
      episode: Number.parseInt(episodeInput.value, 10) || 1
    };
  }
}
...
function buildPlayerUrl(entry) {
  const path = entry.mode === "movie"
    ? `/movie/${encodeURIComponent(entry.id)}`
    : `/tv/${encodeURIComponent(entry.id)}/${entry.season}/${entry.episode}`;
...
function metadataQuery(entry) {
  ...
  return `SELECT ?item ?itemLabel ?itemDescription ?date ?image ?imdb ?movieTmdb ?tvTmdb
    ...
    GROUP BY ?item ?itemLabel ?itemDescription ?date ?image ?imdb ?movieTmdb ?tvTmdb
    LIMIT 1`;
}
...
function bulkMetadataQuery(entries) {
  ...
  return `SELECT ?lookup ?mode ?id ?item ?itemLabel ?itemDescription ?date ?image ?imdb ?movieTmdb ?tvTmdb
    (GROUP_CONCAT(DISTINCT ?genreLabel; separator="|") AS ?genres)
    (GROUP_CONCAT(DISTINCT STR(?genre); separator="|") AS ?genreUris)
    WHERE {
...
async function resolveSelectedList() {
  ...
  if (!entries.length) {
    setStatus("Nothing in this list to resolve.", "warn");
    return;
  }

  const unresolved = entries.filter(entry => !entry.resolvedAt || entry.resolutionStatus !== "resolved");
  if (!unresolved.length) {
    setStatus("Every item in this list is already resolved.", "ok");
    return;
  }
  ...
  try {
    for (let offset = 0; offset < unresolved.length; offset += batchSize) {
      ...
      const bindingsByIndex = new Map(
        data.results.bindings.map(binding => [Number(bindingValue(binding, "lookup")), binding])
      );
    ...
    setStatus(`Bulk resolver stopped after ${completed}: ${error.message}`, "error");
  } finally {
...
function renderCurrent(entry, metadata = null) {
  ...
  configureExternalLink(
    $("#currentImdb"),
    metadata?.imdb ? `https://www.imdb.com/title/${encodeURIComponent(metadata.imdb)}/` : ""
  );
  configureExternalLink(
    $("#currentTmdb"),
    metadata?.tmdb ? `https://www.themoviedb.org/${entry.mode}/${encodeURIComponent(metadata.tmdb)}` : ""
  );
}
...
async function play(entry = currentEntry()) {
  try {
    ...
    if (entry.mode === "tv") {
      ...
      episodeInput.value = entry.episode;
    }
...
async function loadLists() {
  ...
  if (state.selectedList !== "All" && !state.lists.includes(state.selectedList)) state.selectedList = "All";
  await renderListControls();
}
...
async function renderListControls() {
  ...
  for (const name of state.lists) {
    counts.set(name, favorites.filter(entry => entry.list === name).length);
  }

  const chips = ["All", ...state.lists];
  $("#listChips").innerHTML = chips
    .map(name => `<button class="list-chip ${name === state.selectedList ? "active" : ""}" type="button" data-list="${escapeHtml(name)}">${escapeHtml(name)} · ${counts.get(name) || 0}</button>`)
    .join("");
  $("#saveList").innerHTML = state.lists
...
async function addList() {
  ...
  if (!name) {
    setStatus("Enter a list name.", "warn");
    return;
  }
  if (name.toLowerCase() === "all") {
    setStatus('"All" is reserved.', "error");
    return;
  }
...
async function renderLibrary() {
  ...
  if (!entries.length) {
    container.innerHTML = '<div class="empty-list">Nothing saved in this view.</div>';
    return;
  }
...
async function markSelectedListWatched() {
  ...
  const now = new Date().toISOString();
  for (const entry of entries) {
    await putValue(FAVORITES_STORE, { ...entry, watched: true, updatedAt: now });
  }
  await renderLibrary();
...
async function renderContinueWatching() {
  ...
  if (!entries.length) {
    container.innerHTML = '<div class="empty-list">Play a title to add it here.</div>';
    return;
  }
...
function relatedQuery(entry, metadata) {
  ...
  return `SELECT DISTINCT ?item ?itemLabel ?date ?image ?imdb ?tmdb WHERE {
    VALUES ?genre { ${values} }
    ?item wdt:P31/wdt:P279* ${type}; wdt:P136 ?genre.
    FILTER(?item != <${metadata.wikidata}>)
    OPTIONAL { ?item wdt:P577 ?date. }
    OPTIONAL { ?item wdt:P18 ?image. }
    OPTIONAL { ?item wdt:P345 ?imdb. }
    OPTIONAL { ?item ${tmdbProperty} ?tmdb. }
    SERVICE wikibase:label { bd:serviceParam wikibase:language "en". }
  } ORDER BY DESC(?date) LIMIT 16`;
}
...
function renderRelated() {
  ...
  if (!state.related.length) {
    container.innerHTML = '<div class="empty-list">Resolve a title with genre metadata to load related results.</div>';
    return;
  }
...
$("#resolveButton").addEventListener("click", handleAsync(async () => resolveMetadata(currentEntry())));
$("#resolveListButton").addEventListener("click", handleAsync(resolveSelectedList));
$("#markListWatchedButton").addEventListener("click", handleAsync(markSelectedListWatched));
$("#fullscreenButton").addEventListener("click", handleAsync(async () => {
  if (document.fullscreenElement) await document.exitFullscreen();
  else await $("#playerWrap").requestFullscreen();
}));
modeSelect.addEventListener("change", updateModeFields);
mediaIdInput.addEventListener("keydown", event => {
  if (event.key === "Enter") play();
});
$("#addListButton").addEventListener("click", handleAsync(addList));
$("#newListName").addEventListener("keydown", event => {
  if (event.key === "Enter") {
    event.preventDefault();
    addList().catch(error => setStatus(error.message, "error"));
  }
});
$("#listChips").addEventListener("click", event => {
  ...
  Promise.all([renderListControls(), renderLibrary()]).catch(error => setStatus(error.message, "error"));
});
$("#librarySearch").addEventListener("input", () => {
  renderLibrary().catch(error => setStatus(error.message, "error"));
});
$("#libraryCards").addEventListener("click", handleAsync(async event => {
  ...
  if (playButton) {
    const entry = await getValue(FAVORITES_STORE, playButton.dataset.playKey);
    if (entry) await play(entry);
  } else if (resolveButton) {
    await resolveOneFavorite(resolveButton.dataset.resolveKey);
  } else if (watchButton) {
    await toggleWatched(watchButton.dataset.watchKey);
...
$("#continueCards").addEventListener("click", handleAsync(async event => {
  ...
  if (playButton) {
    const entry = await getValue(HISTORY_STORE, playButton.dataset.continuePlay);
    if (entry) await play(entry);
  } else if (doneButton) {
    await completeHistory(doneButton.dataset.continueDone);
  } else if (removeButton) {
    await deleteValue(HISTORY_STORE, removeButton.dataset.continueRemove);
    await renderContinueWatching();
  }
}));
$("#relatedCards").addEventListener("click", async event => {
  ...
  try {
    ...
    setStatus(`ID copied and loaded: ${button.dataset.copyId}`, "ok");
  } catch {
    setStatus(`ID field set to ${button.dataset.copyId}.`, "ok");
  }
});
$("#saveForm").addEventListener("submit", saveCurrentFavorite);
$("#cancelSave").addEventListener("click", () => $("#saveDialog").close());
$("#exportButton").addEventListener("click", handleAsync(exportLibrary));
